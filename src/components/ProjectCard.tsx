  ...
  return (
    <div className="glass-card rounded-2xl p-6 hover:bg-white/10 transition-all duration-300 animate-fade-in group">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-4">
          {project.icon.includes('.') ? (
            <img src={`${import.meta.env.BASE_URL}${project.icon}`} alt={project.name} className="w-14 h-14 rounded-xl shadow-lg" />
          ) : (
            <span className="text-5xl">{project.icon}</span>
          )}
          <div>
            <h3 className="text-xl font-bold text-white font-serif">{project.name}</h3>
            <p className="text-adventist-accent text-sm">{project.category}</p>
          </div>
        </div>
      ...
      <Link
        ...
        className="flex items-center justify-center space-x-2 w-full py-3 rounded-xl bg-adventist-accent text-adventist-dark font-semibold download-btn group-hover:animate-pulse-glow"
      >
