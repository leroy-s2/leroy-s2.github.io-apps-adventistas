      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white font-serif">Apps Adventistas</h1>
                        ...
                        {project.downloads.map((download) => (
                          <a
                            ...
                            title={`${download.platform.charAt(0).toUpperCase() + download.platform.slice(1)} - ${
                              download.status === 'available' ? (download.url ? 'Descargar' : 'Link pendiente') : 'Próximamente'
                            }`}
                          >
            ...
            <div className="mt-2">
              ...
              {isAppsOpen && (
                <div className="pl-4 space-y-2 animate-fade-in">
                  {projects.map((project) => (
                    <div key={project.id} className="glass-card rounded-xl p-3 ml-4 mr-2">
                      <Link
                        to={`/project/${project.id}`}
                      ...
                      <div className="flex gap-2">
                        {project.downloads.map((download) => (
                          ...
                          >
                            {download.platform === 'windows' && <Monitor size={12} />}
                            {download.platform === 'linux' && <LinuxIcon className="w-3 h-3" />}
                            {download.platform === 'mac' && <Apple size={12} />}
                          </a>
                        ))}
                      </div>
                    </div>
